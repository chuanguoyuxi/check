...
def pip_install():
    print("正在安装依赖")
    os.system("pip3 install requests tomli")
...
class config_get(object):
    ...
    def move_configuration_file_old():
        print("移动配置文件")
        os.system(
            "cp /ql/repo/chuanguoyuxi_check/check.sample.toml /ql/config/check.toml"
        )
    ...
    def move_configuration_file_new():
        print("移动配置文件")
        os.system(
            "cp /ql/data/repo/chuanguoyuxi_check/check.sample.toml /ql/data/config/check.toml"
        )
...
class check(object):
    def __init__(
        self, run_script_name, run_script_expression, Configuration_flag=False
    ):
        """
...
def change_cron_new(
    cron_file_path="/ql/data/db/database.sqlite", repositories="chuanguoyuxi_check"
):
    print("尝试修改定时时间")
...
def change_cron_old(
    cron_file_path="/ql/db/crontab.db", repositories="chuanguoyuxi_check"
):
    print("尝试修改定时时间")
